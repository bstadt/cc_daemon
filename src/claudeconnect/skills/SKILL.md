...
```bash
claudeconnect login --remote
```

This automatically:
1. Downloads cloudflared (if needed)
2. Starts a Cloudflare tunnel
3. Prints a login URL to open in any browser
4. Waits for authentication
5. Cleans up the tunnel

For manual tunnel control, use `--redirect-uri` instead:

```bash
claudeconnect login --redirect-uri https://your-tunnel.example.com/callback
```
